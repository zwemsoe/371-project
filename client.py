...
#         while running:
#             game = self.network.send('get_updates')
#             print(game.p1.x)
...
def display_score(game, player):
    font = pygame.font.SysFont('arial', 30)
    score = font.render(f"Score: {game.p1.length}", True, FONT_COLOR)
    if player == 1: 
        score = font.render(f"Score: {game.p2.length}", True, FONT_COLOR)
        
    window.blit(score, (BOARD_SIZE[0] - 150, 10))

def show_game_over(score):
    font = pygame.font.SysFont('arial', 30)
    line1 = font.render(f"Game is over! Your score is {score}", True, FONT_COLOR)
    window.blit(line1, (200, 300))
    line2 = font.render("Close window!", True, FONT_COLOR)
...
def display_game(game, player):
    ...
    if not game.ready:
        font = pygame.font.SysFont("arial", 30)
        text = font.render(f"You are player {player}. Waiting for another player...", 1, (255,0,0), True)
        window.blit(text, (BOARD_SIZE[0]/2 - text.get_width()/2, BOARD_SIZE[1]/2 - text.get_height()/2))
    else:
        # render snakes and stuff
        font = pygame.font.SysFont("arial", 30)
        text = font.render(f"You both joined", 1, (255,0,0), True)
        window.blit(text, (BOARD_SIZE[0]/2 - text.get_width()/2, BOARD_SIZE[1]/2 - text.get_height()/2))
        
        
        display_score(game, player)
...
def main():
    ...
    while not game_over:
        ...
        if game.game_over:
            score = game.p1.length
            if player == 1:
                score = game.p2.length
            game_over = True
